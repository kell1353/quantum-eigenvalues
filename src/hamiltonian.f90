...
real(dp), parameter :: h_bar = 197.3       !MeV/fm
real(dp), parameter :: a = .2              !fm (fermi)
integer, parameter :: mass = 939           !MeV/c^2 (let c=1)
integer, parameter :: potential_depth = 50 !MeV


private

public :: construct_hamiltonian, harmonic_potential_energy, WS_potential_energy, h_bar, mass
contains
...
!!
!! It then constructs the hamiltonian diagonal and off diagonal arrays from the
...
!! Output:
!!
!! hamiltonian_diagonal 		real	Array containing the hamiltonian diagonal constants
!! hamiltonian_off_diagonal 	real	Array containing the hamiltonian off diagonal constants
!-----------------------------------------------------------------------
subroutine construct_hamiltonian(delta, potential_diagonal, hamiltonian_diagonal, hamiltonian_off_diagonal)
implicit none
real(dp), intent(in) :: delta, potential_diagonal(:)
real(dp), intent(out) :: hamiltonian_diagonal(:), hamiltonian_off_diagonal(:)

real(dp), allocatable :: kinetic_diagonal(:), kinetic_off_diagonal(:)
integer :: n
...
!! the harmonic oscillator potential energy into the diagonal array.
!!
!!----------------------------------------------------------------------
!! Input:
!!
...
!-----------------------------------------------------------------------
subroutine harmonic_potential_energy(x, harmonic_potential_diagonal)
implicit none
real(dp), intent(in) :: x(:)
...
do i=1,n
    harmonic_potential_diagonal(i) = ((h_bar**2)/(2*mass))*(x(i)**2)
enddo
